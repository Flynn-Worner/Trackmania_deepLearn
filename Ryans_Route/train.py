...
class StopOnFinishCallback(BaseCallback):
    """Stop training cleanly once any env episode ends with a finish."""
    ...
    def _on_step(self) -> bool:
        ...
        for done, info in zip(dones, infos):
            if not done:
                continue
            if isinstance(info, dict) and info.get("termination_reason") == "finish":
                self.finish_detected = True
                if self.verbose:
                    gates_crossed = info.get("gates_crossed")
                    gates_total = info.get("gates_total")
                    print(
                        "\nFinal gate reached "
                        f"({gates_crossed}/{gates_total}). "
...
def run_training(
    ...
    ----------
    port : int
        telemetry.as socket port for the single running TM window.
    total_timesteps : int
    ...
    print(f"Initializing environment on port {port} ...")
    raw_env = DummyVecEnv([make_env(port)])
    ...
    stop_on_finish_cb = StopOnFinishCallback()
    callback = CallbackList([metrics_cb, autosave_cb, stop_on_finish_cb])
    ...
    try:
        while model.num_timesteps < target_timesteps:
            ...
            try:
                ...
                print(f"\nBridge error during learn: {e}")
                print("Saving checkpoint and stopping training so the session can be restarted cleanly.")
...
def main():
    """Direct execution entry point (single env, default port 9000)."""
    run_training()
