...
.DESCRIPTION
    ...
    This script works by:
      ...
      2. For each desired port, writing a version whose auto-scan starts
         from that port.
...
# Many Windows installs redirect Documents to OneDrive, so check both.
# ---------------------------------------------------------------------------
$documentsCandidates = @(
    "$env:USERPROFILE\OneDrive\Documents",
    "$env:USERPROFILE\Documents"
)
$documentsRoot = $documentsCandidates | Where-Object { Test-Path $_ } | Select-Object -First 1
if (-not $documentsRoot) {
    $documentsRoot = "$env:USERPROFILE\Documents"
}

$pluginsDst = Join-Path $documentsRoot "TMInterface\Plugins"
...
foreach ($port in $Ports) {
    ...
    # Patch the auto-scan loop so this instance starts at its assigned port.
    # The plugin no longer uses RegisterVariable("custom_port").
